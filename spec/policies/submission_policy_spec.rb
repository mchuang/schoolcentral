...
describe SubmissionPolicy do
  before(:each) do
    ...
    @submission0 = FactoryGirl.create(:submission)


    @class0.teachers << @teacher0.account
    @assignment0.submissions << @submission0
  ...
  describe "Admin Scope on Submission" do
    it {expect(SubmissionPolicy::Scope.new(@admin0,Submission).resolve).to eq(Submission.all)}
  end
  #Second test
